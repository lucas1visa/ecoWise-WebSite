...
const NewProduct = () => {
    ...
    const [imageSaved, setImageSaved] = useState(false); // Estado para controlar si la imagen se ha guardado
    ...
    const saveImageUrl = async () => {
        if (imagePreview) {
            try {
                ...
                setImageSaved(true); // Cambiar el estado a verdadero cuando se guarda la imagen
            } catch (error) {
        ...
        <div class="w-full max-w-xs">
            <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" onSubmit={submitHandler}>
                ...
                <div class="flex items-center justify-between">
                    <button class="bg-blue-900 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" type="button" onClick={saveImageUrl}>VALIDATE</button>

                    <button  className={`py-2.5 px-5 mr-2 text-sm font-medium text-gray-100 bg-white-900 rounded-lg border bg-black hover:bg-gray-900 hover:text-blue-700 focus:z-10 focus:ring-4 focus:outline-none focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-900 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-900 inline-flex items-center ${imageSaved ? 'bg-white' : ''}`} type="submit" disabled={!imageSaved}>
                    {!imageSaved ? <svg aria-hidden="true" role="status" class="inline w-4 h-4 mr-3 text-gray-900 animate-spin dark:text-gray-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
